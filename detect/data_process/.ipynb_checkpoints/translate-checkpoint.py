    ...
    with open(output_file, "a+", encoding="utf-8") as f:
        for doc_id, target_lang in doc_with_target_lang:
            ...
            if target_lang == "en":
                ...
                query, human_answer = (
                    docs[doc_id]["query"],
                    docs[doc_id]["human_answer"],
                )
                translated_query = get_response(
                    messages=[
                        {
                            "role": "user",
                            "content": templates.format(
                                language=lang_dict[target_lang], text=query
                            ),
                        }
                    ]
                )
                translated_answer = get_response(
                    messages=[
                        {
                            "role": "user",
                            "content": templates.format(
                                language=lang_dict[target_lang], text=human_answer
                            ),
                        }
                    ]
                )
...
# 使用示例
translate_documents(
    "./data/nq_queries_w_doc.jsonl", "./data/output.jsonl", language_proportions
)
